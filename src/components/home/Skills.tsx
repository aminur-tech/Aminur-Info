...
export default function Skills({ skills: dynamicSkills = [] }: { skills?: Skill[] }) {
  ...
  const iconByCategory = { Frontend: <Globe size={24} />, Backend: <Cpu size={24} />, Database: <Database size={24} />, Tools: <GitBranch size={24} /> };
  const coreSkills = dynamicSkills.length > 0
    ? dynamicSkills.map((skill) => ({ name: skill.name, icon: iconByCategory[skill.category as keyof typeof iconByCategory] || <Code size={24} /> }))
    : fallbackSkills;
