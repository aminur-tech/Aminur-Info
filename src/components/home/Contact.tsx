...
export default function Contact({ profile }: { profile?: Profile }) {
    ...
    const sendEmail = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        setStatus({ type: response.ok ? 'success' : 'error', message: result.message || result.error });
        ...
        <section className="relative overflow-hidden py-20 px-4 transition-colors duration-500 bg-white dark:bg-slate-950" id="contact">
            ...
            <div className="max-w-7xl mx-auto relative z-10">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
                    ...
                    >
                        {[
                            { icon: <Mail />, label: 'Email Me', value: profile?.email || 'aminur.programme@gmail.com' },
                            { icon: <BsWhatsapp />, label: 'WhatsApp Me', value: profile?.phone || '+880 1327 694078' },
                            { icon: <MapPin />, label: 'Location', value: profile?.location || 'Satkhira, Bangladesh' }
                        ].map((item, idx) => (
                    ...
                    >
                        <form onSubmit={sendEmail} className="bg-slate-50 dark:bg-slate-900/40 backdrop-blur-xl border border-slate-200 dark:border-white/5 p-8 md:p-12 rounded-[2.5rem] shadow-2xl space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-2"><label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase ml-2 tracking-widest">Company</label><input type="text" name="company" placeholder="Company or team" className="w-full p-4 rounded-2xl bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-white/5 focus:border-emerald-500/50 text-slate-900 dark:text-white placeholder-slate-400 outline-none" /></div>
                                <div className="space-y-2"><label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase ml-2 tracking-widest">Project type</label><input type="text" name="projectType" placeholder="Web app, redesign, MVP" className="w-full p-4 rounded-2xl bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-white/5 focus:border-emerald-500/50 text-slate-900 dark:text-white placeholder-slate-400 outline-none" /></div>
                            </div>

                            <div className="space-y-2"><label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase ml-2 tracking-widest">Budget range</label><input type="text" name="budgetRange" placeholder="Budget range" className="w-full p-4 rounded-2xl bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-white/5 focus:border-emerald-500/50 text-slate-900 dark:text-white placeholder-slate-400 outline-none" /></div>

                            <div className="space-y-2">
                                <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase ml-2 tracking-widest">Subject</label>
                                <input type="text" name="subject" placeholder="Project Inquiry" className="w-full p-4 rounded-2xl bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-white/5 focus:border-emerald-500/50 focus:ring-0 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-600 transition-all outline-none" required />
                            </div>
