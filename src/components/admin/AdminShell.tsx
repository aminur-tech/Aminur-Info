...
import { 
  Menu, X, LayoutDashboard, UserRound, Sparkles, BriefcaseBusiness, 
  GraduationCap, MessageSquare, Settings, Search, LogOut, Quote, 
  BookOpen, Wrench, Info, Award, Blocks, Link2, ShieldCheck, FileText 
} from "lucide-react";
    ...
    <div className="min-h-screen bg-slate-100 text-slate-900 dark:bg-slate-950 dark:text-slate-100">
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 flex w-64 flex-col border-r border-slate-200 bg-white transition-transform dark:border-slate-800 dark:bg-slate-900 lg:translate-x-0 ${open ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <div className="flex h-16 items-center justify-between border-b border-slate-200 px-5 dark:border-slate-800">
          ...
          >
            <p className="text-[10px] font-bold uppercase tracking-[0.28em] text-emerald-600 dark:text-emerald-400 group-hover:text-emerald-700 dark:group-hover:text-emerald-300 transition-colors">
              Aminur Rahman
            </p>
        ...
        <div className="border-t border-slate-200 p-3 dark:border-slate-800">
          <button
            ...
            className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-semibold transition-colors text-red-600 hover:bg-red-50 hover:text-red-700 dark:text-red-400 dark:hover:bg-red-500/10 dark:hover:text-red-300"
          >
            <LogOut size={18} />
            Log out
          </button>
      ...
      <div className="lg:pl-64">
        ...
        <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-slate-200 bg-white/95 px-4 backdrop-blur dark:border-slate-800 dark:bg-slate-900/95 sm:px-6">
          <div className="flex items-center gap-3">
            ...
            <div className="hidden items-center gap-2 rounded-lg border border-slate-200 px-3 py-2 text-sm text-slate-400 sm:flex dark:border-slate-700">
              <Search size={16} />
              <span>Search content</span>
              <kbd className="ml-8 rounded bg-slate-100 px-1.5 py-0.5 text-[10px] dark:bg-slate-800">⌘K</kbd>
            </div>
            <h1 className="text-sm font-bold sm:hidden">Admin</h1>
          </div>
          
          <div className="flex items-center gap-2">
            <ThemeToggle />
            <div className="hidden text-right sm:block">
              <p className="text-sm font-semibold">Portfolio owner</p>
            </div>
            <div className="grid size-9 place-items-center rounded-full bg-emerald-100 font-bold text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300">
              A
            </div>
          </div>
        </header>
